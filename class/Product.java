...
public class Product{
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException{
			...
			if (isA){
				for( int i = 0; i < n_value; i++){
					word.set( new byte[0] );
					word.set( row_id + "," + i + "," + col_id); // aix -> i, j, x
				...
				for( int i = 0; i < m_value; i++ ){
					word.set( new byte[0] );
					word.set( i + "," + col_id + "," + row_id ); // bxj -> i, j, x
				}
			}
		}
	}
	...
	public static class MatrixMulReducer2 extends
	...
	public static void main(String[] args) throws Exception
        {
        	...
        	String first_phase_result = "/first_phase_result";
                ...
                if (otherArgs.length != 2){
                	System.err.println("usage : Product <in> <out>");
                	System.exit(2);
                }
                
                conf.setInt("m", m_value);
                conf.setInt("k", k_vaule);
                conf.setInt("n", n_value);
                
                Job job1 = new Job(conf, "matrix mult1");
		job1.setJarByClass(Product.class);
		...
		FileSystem.get(job1.getConfiguration()).delete( new Path( first_phase_result ),
		true);
		job1.waitForCompletion(true);
		
		Job job2 = new Job(conf, "matrix mult2");
		job2.setJarByClass(Product.class);
